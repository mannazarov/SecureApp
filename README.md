...
### f.  Brute force
Уязвимость вызвана тем, что пароль состоит ровно их четырёх цифр. 

#### Причины уязвимости:
**Ограниченный набор символов**: Использование только цифр (0-9) существенно ограничивает возможное количество уникальных паролей.

**Короткая длина пароля**: Длина пароля в четыре символа означает, что всего существует 10,000 возможных комбинаций пароля, что является очень небольшим числом.

**Отсутствие защиты от перебора**: Если в приложении нет механизмов для предотвращения или замедления попыток перебора паролей, то злоумышленник может автоматизировать процесс входа в систему и быстро перебрать все возможные комбинации.

#### Как исправить:
**Увеличение сложности пароля**: Следует увеличить минимальную длину пароля и разрешить использование букв (и заглавных, и строчных), цифр и специальных символов. Это значительно увеличит количество возможных комбинаций.
